...
export default function Game() {
  ...
  useEffect(() => {
    socket.on("state", (s) => {
      ...
      if (s.players && Array.isArray(s.players)) {
        ...
        const playersChanged =
          currentPlayerIds.length !== previousPlayerIds.length ||
    ...
    // Si ya está conectado, usa socketId del hook
    setMe(socketId);
  ...
  const getCardSpecificFigures = useCallback(
    (cardIndex) => {
      if (!me || cardIndex === undefined) return [];
      return Bingo.getCardSpecificFigures(
        me,
        cardIndex,
        state.specificClaims || {}
      );
    },
  ...
  const toggle = useCallback((cardIndex, r, c) => {
    ...
    InteractionManager.runAfterInteractions(() => {
      setLocalMarks((prev) => {
        ...
        const grid = prev[cardIndex]
          ? prev[cardIndex].map((row) => row.slice())
  ...
  const claimAutoAll = useCallback(() => {
    ...
    try {
      ...
      const buildEffectiveMarked = (card, localMarked) => {
        ...
        const m = Array.from({ length: 5 }, (_, r) =>
          Array.from({ length: 5 }, (_, c) => false)
        );
        for (let r = 0; r < 5; r++) {
          for (let c = 0; c < 5; c++) {
            if (r === 2 && c === 2) {
      ...
      for (let idx = 0; idx < cards.length; idx++) {
        ...
        const completedFigures = Object.entries(figures)
          .filter(([key, value]) => value)
          .map(([key]) => key);
        const availableFigures = completedFigures.filter(
          (fig) => !state.figuresClaimed?.[fig]
        );
  ...
  // 🐛 Función de DEBUG para simular figuras ganadas
  const debugClaimFigure = useCallback(
    (figure) => {
      ...
      console.log(`🐛 DEBUG: Simulando claim de figura: ${figure}`);

      socket.emit("claim", {
        roomId: rid,
        figure: figure,
        cardIndex: 0, // Primera carta
        marked: [], // No importa en modo debug
        debugMode: true, // 🐛 Indicar al backend que es modo debug
      });
    },
    [state.roomId, params.roomId, isClaiming]
  );
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <Ionicons
              name={musicMuted ? "musical-notes-outline" : "musical-notes"}
              size={20}
              color={musicMuted ? "#95a5a6" : "#2c3e50"}
            />
          </TouchableOpacity>
          <TouchableOpacity
          ...
          >
            <Ionicons
              name={effectsMuted ? "volume-mute" : "volume-high"}
              size={20}
          ...
          {(() => {
            ...
            const renderCards = () => {
              ...
              if (count === 1) {
                ...
                const computedAspect = Math.min(
                  1.3,
                  Math.max(0.9, widthTarget / maxCardHeight)
                );
              ...
              if (count === 2) {
                ...
                const computedAspect = Math.min(
                  1.5,
                  Math.max(1.0, cardWidth / maxCardHeight)
                );
                  ...
                  >
                    <View style={{ width: cardWidth }}>
                      {cards.map((card, i) => (
                        <View
                          key={i}
                          style={{ marginBottom: i === 0 ? spacing : 0 }}
                        >
                          <Bingo.BingoCard
                            card={card}
                            drawn={state.drawn}
