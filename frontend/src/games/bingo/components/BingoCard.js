...
function BingoCardBase({
  ...
  const isInCompletedFigure = (r, c) => {
    return (specificFigures || []).some((entry) => {
      const fig =
        typeof entry === "string" ? { figure: entry, details: {} } : entry;
        ...
        {card.map((row, r) => (
          <View key={r} style={{ flexDirection: "row" }}>
            {row.map((n, c) => {
              ...
              const inCompletedFigure =
                isInCompletedFigure(r, c) && isCellMarked;
                ...
                >
                  ...
                  {isCenter ? (
                    <Image
                      ...
                      style={{
                        width: sizes.cellFontSize * 1.1,
                        height: sizes.cellFontSize * 1.1,
                        resizeMode: "contain",
                      }}
                    />
                  ) : (
...
export const BingoCard = React.memo(BingoCardBase, (prev, next) => {
  return (
    prev.card === next.card &&
    ...
    JSON.stringify(prev.specificFigures || []) ===
      JSON.stringify(next.specificFigures || [])
  );
});
